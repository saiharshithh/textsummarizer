...
if st.button("Summarize") and input_data:
    with st.spinner("Summarizing... Please wait."):
        inputs = tokenizer.encode("summarize: " + input_data, return_tensors="pt", max_length=512, truncation=True)
        outputs = model.generate(inputs, max_length=150, min_length=30, length_penalty=2.0, num_beams=4,
                                 early_stopping=True)
        summary = tokenizer.decode(outputs[0], skip_special_tokens=True)
        st.text_area("Summary:", value=summary, height=200)

        # Prepare summary for copying
        safe_summary = summary.replace("'", "\\'").replace('"', '\\"')[:500]  # Truncate if needed
        ...
        st.markdown(f"""
            ...
            <button class="copy-btn" onclick="navigator.clipboard.writeText('{safe_summary}')">Copy Summary</button>
        """, unsafe_allow_html=True)
else:
    st.info("Please provide input data to summarize.")
